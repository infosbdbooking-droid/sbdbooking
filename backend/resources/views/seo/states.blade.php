    ...
    <div class="container-fluid">
                        ...
                        <ol class="inline-flex items-center space-x-1 md:space-x-2">
                            ...
                            <li>
                                <div class="flex items-center">
                                    ...
                                    <span class="text-gray-900 font-medium">States</span>
                                </div>
                            </li>
                        </ol>
                    </nav>
                    <h1 class="text-xl font-bold text-gray-900">States Master</h1>
                </div>
        ...
        <div id="Modal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50">
            <div class="bg-white w-full max-w-lg mx-auto p-6 rounded-lg shadow-lg relative">
                ...
                <h2 class="text-lg font-bold mb-4" id="modalTitle">Add State</h2>
                <form id="dataForm" action="{{ route('seoStates.store') }}" method="POST">
                    @csrf
                    <div class="mb-4"> 
                        <label class="block text-sm font-medium text-gray-700 mb-1">State Name <span class="text-red-500">*</span></label>
                        <input type="text" name="state_name" id="state_name"
                            placeholder="e.g. Delhi, Maharashtra, Goa..."
                    ...
                    <div class="mb-4"> 
                        ...
                        <input type="text" name="slug" id="state_slug" readonly
                            class="w-full border border-gray-100 bg-gray-50 text-gray-500 rounded px-3 py-2 focus:outline-none">
                    </div>
                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Status</label>
                        <select name="status" id="state_status" class="w-full border border-gray-300 rounded px-3 py-2 focus:ring-1 focus:ring-blue-500 focus:outline-none">
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#dataForm').on('submit', function (e) {
                ...
                const url = isEditing ? `{{ url('panel/seo-states') }}/${editingId}` : $(form).attr('action');
            ...
            $('#table tbody').on('click', '.edit-btn', function () {
                ...
                $.get(`{{ url('panel/seo-states') }}/${editingId}/edit`, function (data) {
                    $('#state_name').val(data.state_name);
                    $('#state_slug').val(data.slug);
                    $('#state_status').val(data.status);
                    $('#modalTitle').text('Edit State');
                    $('.addBtn').text('Update State');
                    $('#Modal').removeClass('hidden').addClass('flex');
                });
            });
            ...
            $('#table tbody').on('click', '.delete-btn', function () {
                ...
                Swal.fire({
                    ...
                    html: '<p class="text-gray-700 text-sm">Are you sure you want to delete this state?</p>',
                    icon: 'warning',
                    width: 350,
            ...
            $('#table tbody').on('click', '.toggle-status', function () {
                ...
                $.ajax({
                    url: `{{ url('panel/seo-states') }}/${id}/changeStatus`,
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        status: newStatus
