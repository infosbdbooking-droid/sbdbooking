    ...
    <div class="container-fluid">
                        ...
                        <ol class="inline-flex items-center space-x-1 md:space-x-2">
                            ...
                            <li>
                                <div class="flex items-center">
                                    ...
                                    <span class="text-gray-900 font-medium">FAQs Master</span>
                                </div>
                            </li>
                        </ol>
                    </nav>
                    <h1 class="text-xl font-bold text-gray-900">FAQ Master</h1>
                </div>
        ...
        <div id="Modal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50">
            <div class="bg-white w-full max-w-lg mx-auto p-6 rounded-lg shadow-lg relative">
                ...
                <h2 class="text-lg font-bold mb-4" id="modalTitle">Add FAQ</h2>
                <form id="dataForm" action="{{ route('seoFaqs.store') }}" method="POST">
                    @csrf
                    <div class="mb-4"> 
                        ...
                        <input type="text" name="question" id="faq_question"
                            placeholder="e.g. What is the distance between Delhi and Agra?"
                            class="w-full border border-gray-300 rounded px-3 py-2 text-xs focus:ring-1 focus:ring-blue-500 focus:outline-none">
                    </div>
                    <div class="mb-4"> 
                        <label class="block text-xs font-semibold text-gray-700 mb-1">Answer <span class="text-red-500">*</span></label>
                        <textarea name="answer" id="faq_answer" rows="4" placeholder="Enter answers here..."
                    ...
                    <div class="mb-4">
                        ...
                        <select name="status" id="faq_status" class="w-full border border-gray-300 rounded px-3 py-2 text-xs focus:ring-1 focus:ring-blue-500 focus:outline-none">
    ...
    <script>
        $(document).ready(function () {
            ...
            const table = $('#table').DataTable({
                ...
                ajax: {
                    url: '{{ route("seoFaqs.data") }}',
                    type: 'GET',
                    dataType: 'json',
                    error: function (xhr) {
                        $.toastr.error('Error fetching FAQs.');
                    }
                },
                columns: [
                    {
            ...
            $('#dataForm').on('submit', function (e) {
                ...
                const url = isEditing ? `{{ url('panel/seo-faqs') }}/${editingId}` : $(form).attr('action');
            ...
            $('#table tbody').on('click', '.edit-btn', function () {
                ...
                $.get(`{{ url('panel/seo-faqs') }}/${editingId}/edit`, function (data) {
                    $('#faq_question').val(data.question);
                    $('#faq_answer').val(data.answer);
                    $('#faq_status').val(data.status);
                    $('#modalTitle').text('Edit FAQ');
                    $('.addBtn').text('Update FAQ');
                    $('#Modal').removeClass('hidden').addClass('flex');
                });
            });
            ...
            $('#table tbody').on('click', '.delete-btn', function () {
                ...
                Swal.fire({
                    ...
                    html: '<p class="text-gray-700 text-sm">Are you sure you want to delete this FAQ?</p>',
                    icon: 'warning',
                    width: 350,
            ...
            $('#table tbody').on('click', '.toggle-status', function () {
                ...
                $.ajax({
                    url: `{{ url('panel/seo-faqs') }}/${id}/changeStatus`,
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        status: newStatus
