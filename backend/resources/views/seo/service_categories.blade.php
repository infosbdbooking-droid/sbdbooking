    ...
    <div class="container-fluid">
                        ...
                        <ol class="inline-flex items-center space-x-1 md:space-x-2">
                            ...
                            <li>
                                <div class="flex items-center">
                                    ...
                                    <span class="text-gray-900 font-medium">Service Categories</span>
                                </div>
                            </li>
                        </ol>
                    </nav>
                    <h1 class="text-xl font-bold text-gray-900">Service Categories Master</h1>
                </div>
        ...
        <div id="Modal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50">
            <div class="bg-white w-full max-w-lg mx-auto p-6 rounded-lg shadow-lg relative">
                ...
                <h2 class="text-lg font-bold mb-4" id="modalTitle">Add Service Category</h2>
                <form id="dataForm" action="{{ route('seoServiceCategories.store') }}" method="POST">
                    @csrf
                    <div class="mb-4"> 
                        <label class="block text-sm font-medium text-gray-700 mb-1">Category Name <span class="text-red-500">*</span></label>
                        <input type="text" name="category_name" id="category_name"
                            placeholder="e.g. Cab Booking, Taxi Service..."
                    ...
                    <div class="mb-4"> 
                        ...
                        <input type="text" name="slug" id="category_slug" readonly
                            class="w-full border border-gray-100 bg-gray-50 text-gray-500 rounded px-3 py-2 focus:outline-none">
                    </div>
                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Status</label>
                        <select name="status" id="category_status" class="w-full border border-gray-300 rounded px-3 py-2 focus:ring-1 focus:ring-blue-500 focus:outline-none">
    ...
    <script>
        $(document).ready(function () {
            ...
            const table = $('#table').DataTable({
                ...
                ajax: {
                    url: '{{ route("seoServiceCategories.data") }}',
                    type: 'GET',
                    dataType: 'json',
                    error: function (xhr) {
                        $.toastr.error('Error fetching service categories.');
                    }
                },
                columns: [
                    {
            ...
            $('#dataForm').on('submit', function (e) {
                ...
                const url = isEditing ? `{{ url('panel/seo-service-categories') }}/${editingId}` : $(form).attr('action');
            ...
            $('#table tbody').on('click', '.edit-btn', function () {
                ...
                $.get(`{{ url('panel/seo-service-categories') }}/${editingId}/edit`, function (data) {
            ...
            $('#table tbody').on('click', '.delete-btn', function () {
                ...
                Swal.fire({
                    ...
                    html: '<p class="text-gray-700 text-sm">Are you sure you want to delete this service category?</p>',
                    icon: 'warning',
                    width: 350,
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: `{{ url('panel/seo-service-categories') }}/${id}`,
                            type: 'POST',
                            data: {
                                _method: 'DELETE',
                                _token: '{{ csrf_token() }}'
            ...
            $('#table tbody').on('click', '.toggle-status', function () {
                ...
                $.ajax({
                    url: `{{ url('panel/seo-service-categories') }}/${id}/changeStatus`,
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        status: newStatus
