    ...
    <div class="container-fluid">
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                        ...
                        <ol class="inline-flex items-center space-x-1 md:space-x-2">
                            ...
                            <li>
                                <div class="flex items-center">
                                    ...
                                    <span class="text-gray-900 font-medium">Blog Categories</span>
                                </div>
                            </li>
                        </ol>
                    </nav>
                ...
                <div class="flex flex-wrap gap-2">
                    <button type="button" id="addModalBtn"
                        class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-md flex items-center transition-colors shadow">
        ...
        <div id="Modal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50">
            <div class="bg-white w-full max-w-lg mx-auto p-6 rounded-lg shadow-lg relative">
                ...
                <h2 class="text-lg font-bold mb-4" id="modalTitle">Add Category</h2>
                <form id="dataForm" action="{{ route('blogCategories.store') }}" method="POST">
                    @csrf
                    <div class="mb-4"> 
                        <label class="block text-sm font-medium text-gray-700 mb-1">Category Name <span class="text-red-500">*</span></label>
                        <input type="text" name="category_name" id="category_name"
                            placeholder="e.g. Travel Guide, Food, Hotels..."
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#dataForm').on('submit', function (e) {
                ...
                const url = isEditing ? `{{ url('panel/blog-categories') }}/${editingId}` : $(form).attr('action');
            ...
            $('#table tbody').on('click', '.edit-btn', function () {
                ...
                $.get(`{{ url('panel/blog-categories') }}/${editingId}/edit`, function (data) {
            ...
            $('#table tbody').on('click', '.delete-btn', function () {
                ...
                Swal.fire({
                    ...
                    html: '<p class="text-gray-700 text-sm">Are you sure you want to delete this blog category?</p>',
                    icon: 'warning',
                    width: 350,
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: `{{ url('panel/blog-categories') }}/${id}`,
                            type: 'POST',
                            data: {
                                _method: 'DELETE',
                                _token: '{{ csrf_token() }}'
            ...
            $('#table tbody').on('click', '.toggle-status', function () {
                ...
                $.ajax({
                    url: `{{ url('panel/blog-categories') }}/${id}/changeStatus`,
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        status: newStatus
